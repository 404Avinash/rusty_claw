...
def review_evidence(intent: IntentObject) -> str:
    return (
        f"Evidence Review ({intent.case_id}): FIR reviewed. Arrest memo analyzed. "
        f"Key gaps identified in prosecution's case. Potential for bail on merits. "
        f"Recommend motion for disclosure of documents under Section 207 Cr.P.C."
    )
...
def send_legal_notice(intent: IntentObject) -> str:
    _ensure_output()
    filename = f"output/legal_notice_{intent.case_id}.txt"
    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"LEGAL NOTICE\nTo: {intent.target}\nRe: {intent.content}\n"
...
def summarize_precedents(intent: IntentObject) -> str:
    return f"Precedent Summary: Analyzed case law for: '{intent.content}'"


# ─────────────────────────────────────────────
# GATED TOOL (should only reach here if policy allows)
# ─────────────────────────────────────────────
...
"""
    if "landlord" in c or "illegal entry" in c or "rent" in c:
        body = (
            ...
            "You are hereby notified that your unauthorized entry into the demised premises "
            ...
            "DEMANDS:\n"
            "1. Immediately cease all unauthorized entries into the premises.\n"
            "2. Provide written assurance of non-repetition within 7 days.\n"
            "3. Failure to comply will result in a complaint under IPC Section 441 (Criminal Trespass) "
        ...
        body = "Refer to the bail application document for the full pleading."
    else:
...
def _get_precedents(query: str) -> list[str]:
    ...
    if any(k in q for k in ["landlord", "tenant", "rent", "entry"]):
        return [
            "Satyawati v. Municipal Corporation (2018) — Unauthorized entry by landlord void under TPA",
    ...
    if any(k in q for k in ["termination", "employment", "wages", "salary", "labour"]):
        return [
            "Bharatiya Kamgar Karmachari v. State (2020) — Wrongful termination remedy: reinstatement + back wages",
    ...
    if any(k in q for k in ["bail", "arrest", "fraud", "criminal", "crpc"]):
        return [
            "Arnesh Kumar v. State of Bihar (2014 SC) — Supreme Court: arrest not automatic; conditions must be met",
            "Sanjay Chandra v. CBI (2012 SC) — Economic offence not ground to deny bail if trial delayed",
